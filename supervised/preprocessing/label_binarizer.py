...
class LabelBinarizer(object):
    ...
    def transform(self, X, column):
        if len(self._uniq_values) == 2:
            X[column + '_' + str(self._uniq_values[1])] = (X[column] == self._uniq_values[1]).astype(int)
        else:
            for v in self._uniq_values:
    ...
    def to_json(self):
        self._uniq_values = [i if type(i) != np.bool_ else bool(i) for i in list(self._uniq_values)]
        data_json = {"new_columns": list(self._new_columns),
