...
from preprocessing.preprocessing_missing import PreprocessingMissingValues
...
class PreprocessingMissingValuesTest(unittest.TestCase):
    ...
    def test_preprocessing_constructor(self):
        ...
        self.assertEqual(preprocess_missing._na_fill_method, PreprocessingMissingValues.FILL_NA_MEDIAN)
    ...
    def test_get_fill_value(self):
        ...
        self.assertEqual(preprocess_missing._get_fill_value(df['col2']), 'a')
        # fill with min
        preprocess_missing = PreprocessingMissingValues(PreprocessingMissingValues.FILL_NA_MIN)
        self.assertEqual(preprocess_missing._get_fill_value(df['col1']), 0)
        self.assertEqual(preprocess_missing._get_fill_value(df['col2']), '_missing_value_') # added new value
    ...
    def test_fit_na_fill(self):
        ...
        # fill with mean
        preprocess_missing = PreprocessingMissingValues(PreprocessingMissingValues.FILL_NA_MEAN)
        preprocess_missing._fit_na_fill(df)
        self.assertTrue('col1' in preprocess_missing._na_fill_params)
        self.assertTrue('col2' in preprocess_missing._na_fill_params)
        self.assertTrue('col3' not in preprocess_missing._na_fill_params)
    ...
    def test_transform(self):
        ...
        self.assertEqual(2, len(preprocess_missing._na_fill_params)) # there should be only two columns
        df_transformed = preprocess_missing.transform(df_test)
        self.assertTrue(np.isnan(df.loc[3, 'col1'])) # training data frame is not filled
        self.assertEqual(2, df_test.loc[3, 'col1']) # data frame is filled after transform
    ...
    def test_transform_on_new_data(self):
        ...
        pm = PreprocessingMissingValues(PreprocessingMissingValues.FILL_NA_MEDIAN)
        pm.fit(X_train)
