...
class TimeConstraint(Callback):
    ...
    def on_iteration_end(self, logs, predictions):
        ...
        print(
            "Iteration took {} seconds, last iter {}, total training time {} seconds".format(
                time.time() - self.iter_start_time, self.last_iteration_time, time.time() - self.train_start_time
            )
        )
        ...
        if time.time() - self.train_start_time + self.last_iteration_time > self.train_time_limit:
            self.learner.stop_training = True
            log.info("Terminating learning, time limit will be exceeded")
            print(">>> Terminating learning, time limit will be exceeded")
