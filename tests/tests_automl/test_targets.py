...
class AutoMLTargetsTest(unittest.TestCase):
    ...
    def test_bin_class_01(self):
        ...
        automl = AutoML(results_path=self.automl_dir, total_time_limit=1, algorithms=["Xgboost"], train_ensemble=False)
    ...
    def test_bin_class_AB_missing_targets(self):
        X = np.random.rand(self.rows,3)
        X = pd.DataFrame(X, columns=[f"f{i}" for i in range(3)])
        y = pd.Series(np.random.permutation(['a', 'B']*int(self.rows/2)), name='target')
    ...
    def test_multi_class_0123(self):
        ...
        for col in ["prediction_0", "prediction_1", "prediction_2", "prediction_3", "label"]:
            self.assertTrue(col in pred.columns.tolist())
        u = np.unique(pred["label"].values)
    ...
    def test_multi_class_abcd(self):
        X = np.random.rand(self.rows*4,3)
        X = pd.DataFrame(X, columns=[f"f{i}" for i in range(3)])
        y = pd.Series(np.random.permutation(['a', 'B', 'CC', 'd']*self.rows), name='target')
        ...
        for col in ["prediction_a", "prediction_B", "prediction_CC", "prediction_d", "label"]:
            self.assertTrue(col in pred.columns.tolist())
        u = np.unique(pred["label"].values)
